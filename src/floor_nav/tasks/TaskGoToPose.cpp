...
{
	if(cfg.Holonomic){ //HOLONOMIC MODE
		...
		double velx = cfg.k_v * (x_init + cfg.goal_x-tpose.x);
		if (velx > cfg.max_velocity) velx = cfg.max_velocity;
		
		double vely = cfg.k_v * (y_init + cfg.goal_y-tpose.y);
		if (vely > cfg.max_velocity) vely = cfg.max_velocity;
		...
		double rot = std::max(std::min(cfg.k_alpha*gamma,cfg.max_angular_velocity),-cfg.max_angular_velocity);//Clacul a small rotation
